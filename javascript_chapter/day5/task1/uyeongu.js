...
function question(str) {
  ...
  let rm_special_character = str.replace(reg, "");
  let rm_blank = rm_special_character.replace(blank, "");
  let lower_str = rm_blank.toLowerCase();
  let slice_str1, slice_str2;

  let half = lower_str.length / 2;

  if (lower_str.length % 2 === 0) {
    slice_str1 = lower_str.slice(0, half);
  } else {
    slice_str1 = lower_str.slice(0, half + 1);
  }
  slice_str2 = lower_str.slice(half, lower_str.length);

  let arr_str = [...slice_str2];
  let reverse_arr = arr_str.reverse();
  slice_str2 = reverse_arr.join("");

  if (slice_str1 == slice_str2) {
    result = true;
  } else {
    result = false;
  }
